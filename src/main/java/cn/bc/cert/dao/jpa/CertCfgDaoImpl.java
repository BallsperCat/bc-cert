...
@Component
public class CertCfgDaoImpl extends JpaCrudDao<CertCfg> implements CertCfgDao {
  @Autowired
  ...
  public List<Map<String, String>> find4Option(Integer[] statuses, String[] typeCodes) {
    ...
    return executeNativeQuery(hql, args, new RowMapper<Map<String, String>>() {
  ...
  public List<Map<String, Object>> find4AllCertsNameAndIdCfgByTypeCode(String typeCode) {
    String sql = "";
    if (typeCode == null) {
      sql = " select f.name,f.id from bc_cert_cfg f inner join bc_cert_type t on f.type_id = t.id where f.status_ = 0 order by f.order_no asc ";
    } else {
