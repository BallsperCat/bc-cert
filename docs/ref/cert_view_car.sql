...
with actor(id) as (
  ...
  -- 有权限查阅的证件
    ...
    where (
            ...
            exists(select 0
                   from acl
                   where acl.cert_id = c.id and acl.role_ in ('10', '11', '01'))
            -- 无 ACL 权限控制
            or not exists(select 0
                          from acl
                          where acl.cert_id = c.id)
          )
          -- 没有 ACL 的限制权限
          and not exists(select 0
                         from acl
                         where acl.cert_id = c.id and acl.role_ = '00')
  ),
  -- 车辆+证件
...
select t.* -- 查询函数
  ...
  (select d.f_value
   from field d
   where d.car_id = t.car_id and d.f_code = t.c_name and d.f_name = 'attach_id')          attach_id0,
...
from (
       select cc.car_id  car_id, cc.plate_type plate_type, cc.plate_no plate_no, cc.t_name t_name, cc.c_name c_name,
              cc.c_order c_order, cc.file_date file_date
