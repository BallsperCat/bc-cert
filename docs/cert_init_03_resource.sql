...
-- 资源配置：证件类别800601
insert into BC_IDENTITY_RESOURCE (ID, STATUS_, INNER_, TYPE_, BELONG, ORDER_, NAME, URL, ICONCLASS, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '800601', '证件类别', '/modules/bc/cert/certTypes/paging', 'i0001',
    '证件管理'
  from BC_IDENTITY_RESOURCE m
...
-- 资源配置：证件配置800602
insert into BC_IDENTITY_RESOURCE (ID, STATUS_, INNER_, TYPE_, BELONG, ORDER_, NAME, URL, ICONCLASS, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '800602', '证件配置', '/modules/bc/cert/certCfgs/paging', 'i0003',
    '证件管理'
  from BC_IDENTITY_RESOURCE m
  where m.order_ = '800600'
        and not EXISTS(select 1
...
-- 资源配置：证件信息800603
insert into BC_IDENTITY_RESOURCE (ID, STATUS_, INNER_, TYPE_, BELONG, ORDER_, NAME, URL, ICONCLASS, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '800603', '证件信息', '/modules/bc/cert/certInfos/paging', 'i0003',
    '证件管理'
  from BC_IDENTITY_RESOURCE m
  where m.order_ = '800600'
        and not EXISTS(select 1
...
insert into BC_IDENTITY_ROLE (ID, STATUS_, INNER_, TYPE_, ORDER_, CODE, NAME)
  ...
  where not exists(select 1
                   ...
                   where code = 'BC_CERT_MANAGE');

--	增加证件管理岗位CertManageGroup
insert into BC_IDENTITY_ACTOR (ID, UID_, STATUS_, INNER_, TYPE_, CODE, NAME, ORDER_, PCODE, PNAME)
  select NEXTVAL('CORE_SEQUENCE'), 'group.cert.manage' || NEXTVAL('CORE_SEQUENCE'), 0, false, 3, 'CertManageGroup',
    '证件管理岗', '11001', '[1]baochengzongbu', '宝城'
  from BC_DUAL
  where not exists(select 0
                   from BC_IDENTITY_ACTOR
                   where CODE = 'CertManageGroup');
...
insert into BC_IDENTITY_ROLE_ACTOR (RID, AID)
  ...
  where r.CODE = 'BC_CERT_MANAGE'
        ...
        and not exists(select 0
                       ...
                       where ra.RID = r.id and ra.AID = a.id);
  ...
  where r.code = 'BC_CERT_MANAGE'
        ...
        and not exists(select 1
                       ...
                       where rid = (select r2.id
                                    ...
                                    where r2.code = 'BC_CERT_MANAGE')
                             and sid = (select m2.id
                                        from BC_IDENTITY_RESOURCE m2
                                        where m2.type_ > 1 and m2.order_ in ('800601')))
  order by m.order_;
  ...
  where r.code = 'BC_CERT_MANAGE'
        ...
        and not exists(select 1
                       ...
                       where rid = (select r2.id
                             ...
                             and sid = (select m2.id
                                        from BC_IDENTITY_RESOURCE m2
                                        where m2.type_ > 1 and m2.order_ in ('800602')))
  order by m.order_;
  ...
  where r.code = 'BC_CERT_MANAGE'
        ...
        and not exists(select 1
                       ...
                       where rid = (select r2.id
                             ...
                             and sid = (select m2.id
                                        from BC_IDENTITY_RESOURCE m2
                                        where m2.type_ > 1 and m2.order_ in ('800603')))
  order by m.order_;
...
insert into BC_TEMPLATE (ID, UID_, STATUS_, ORDER_, CATEGORY, CODE, VERSION_, FORMATTED, INNER_, PATH, SIZE_, SUBJECT, DESC_, TYPE_ID, FILE_DATE, AUTHOR_ID)
  select NEXTVAL('CORE_SEQUENCE'), 'Template.cmc.0', 0, '6000', '证件模板', 'DEFAULT_CERT_FORM', '1', true, false,
    '/bc/default.cert.form.ftl', 25000, '默认证件表单模板', '', (select id
                                                         from BC_TEMPLATE_TYPE
                                                         where code = 'freemarker'), now(), (select id
                                                                                             from
                                                                                               BC_IDENTITY_ACTOR_HISTORY
                                                                                             where
                                                                                               actor_name = '系统管理员' and
                                                                                               current = true)
  from bc_dual
  where not exists(select 1
                   from bc_template
                   where uid_ = 'Template.cmc.0');
...
insert into bc_option_item (id, pid, key_, value_, order_, status_, desc_)
  select NEXTVAL('hibernate_sequence'), (select id
                                         ...
                                         where key_ = 'carman.cert' and value_ = '司机证件'), 'cert.Cfg.defaultTplCode',
    'DEFAULT_CERT_FORM', '00', 0, '默认的证件模板'
  from bc_dual
  where not exists(select 0
                   from bc_option_item
                   where key_ = 'cert.Cfg.defaultTplCode' and pid = (select id
                                                                     from bc_option_group
                                                                     where key_ = 'carman.cert' and value_ = '司机证件'));
...
insert into BC_IDENTITY_ROLE_RESOURCE (RID, SID)
  ...
  where r.code = 'BC_CERT_ACL_MANAGE'
        ...
        and not exists(select 1
                       ...
                       where rid = (select r2.id
                                    from BC_IDENTITY_ROLE r2
                                    where r2.code = 'BC_CERT_ACL_MANAGE')
                             and sid = (select m2.id
                                        from BC_IDENTITY_RESOURCE m2
                                        where m2.type_ > 1 and m2.order_ in ('800602')))
  order by m.order_;
